...
intermediateICM = False # trace intemediate ICM cells to terminal ones
pheno_t = True # plot data separated into classes
...
nanog_analysis = True # do analysis on nanog stochasticity
if nanog_analysis:
    nanog_thresh = 0.2
    nanog_on = []
    nanog_on_minsave = -75
...
for given_stack in given_stacks:
    ...
    if len(pk_file) < 1 or force_reprocessing:
        ...
        # trace intermediate ICM labels to terminal cells
        if intermediateICM:
            ICM = []
            termICM = []
            with open(os.path.join(stack_directory, 'ICM_at_frame_124.csv'), newline='') as csvfile:
                reader = csv.reader(csvfile, delimiter=',', quotechar='|')
                for row in reader:
                    s1 = row[0].replace("'", '')
                    s1_1 = int(s1[0:s1.find('_')])
                    ...
                    ICM.append('<' + str(s1_1) + '_' + str(s1_2) + '<')
        ...
        for cell in allcells:
            ...
            if intermediateICM:
                for ICMcell in ICM:
                    if ICMcell in lin[0]:
                        termICM.append(lin[0][0:lin[0].find('<')])
    ...
    if pheno_t: # plot pheonotype-specific plots
        ...
        for p in pheno:
            ...
            for cell in temp_cells:
                lin = lineage(cell[0], cell[1])
                temp_data.append([cell, express(transform(lin[0])), lin[1]])
    ...
    if plot:
        ...
        if pheno_t:
            ...
            for p in range(len(pheno)):
                ...
                temp = []
                for i in range(stack[1]):
                    temp.append((stack[2][i][1][len(stack[2][i][1])-1, 3] + 0.000001 - red_min) / (red_max - red_min))
                
                sns.swarmplot(x = p, y = temp, color = colsel(pheno_data[p][0]), label = pheno_data[p][0])
            plt.ylim(0, math.pi/2)
            plt.title(given_stack + ' terminal Nanog distribution')
            ...
            """ do nanog stochasticity analysis """
            ...
            if nanog_analysis:
                ...
                for pheno in pheno_data:
                    temp_data = pheno[1][2] # has elements of [terminal cell, traces, div frames]
                    for cell in temp_data:
                        ...
                        for frame in trace:
                            nanog = (frame[3] - red_min)/(red_max - red_min)
                            right_stage = False
                            if (frame[0] >= divs[0][0]) and (frame[0] <= divs[1][0]):
                                right_stage = True
                                
                            if (nanog >= nanog_thresh) and not (on):
                                if ((frame[0] - divs[0][0])/(divs[1][0] - divs[0][0])) < nanog_on_minsave:
                                    save = False
                                    on = True
                                else:
                                    nanog_on_tmp.append((frame[0] - divs[0][0])/(divs[1][0] - divs[0][0]))
                                    on = True

                            
                            if right_stage:
                                temp_trace.append(nanog)
                                temp_t1.append((frame[0] - divs[0][0])/4)
                                temp_t2.append((divs[1][0] - frame[0])/4)
                        
                        if save:
                            cell_cycle_tmp.append(divs[1][0] - divs[0][0])
                            nanog_peak_tmp.append(np.max(temp_trace))
                            nanog_avg_tmp.append(np.mean(temp_trace))
                            nanog_traces_tmp.append([temp_trace, temp_t1, temp_t2])
                
                # plot stack-specific correlations
                plt.scatter(nanog_on_tmp, nanog_peak_tmp)
                plt.xlabel('Nanog on frame')
                plt.ylabel('Nanog peak')
                cor = scipy.stats.pearsonr(nanog_on_tmp, nanog_peak_tmp)
                plt.title(given_stack + ' R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
                ...
                plt.scatter(nanog_avg_tmp, nanog_peak_tmp)
                plt.xlabel('Nanog average')
                plt.ylabel('Nanog peak')
                cor = scipy.stats.pearsonr(nanog_avg_tmp, nanog_peak_tmp)
                plt.title(given_stack + ' R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
                ...
                plt.scatter(nanog_on_tmp, nanog_avg_tmp)
                plt.xlabel('Nanog on frame')
                plt.ylabel('Nanog average')
                cor = scipy.stats.pearsonr(nanog_on_tmp, nanog_avg_tmp)
                plt.title(given_stack + ' R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
                ...
                plt.scatter(nanog_on_tmp, cell_cycle_tmp)
                plt.xlabel('Nanog on frame')
                plt.ylabel('Cell cycle (frames)')
                cor = scipy.stats.pearsonr(nanog_on_tmp, cell_cycle_tmp)
                plt.title(given_stack + ' R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
                ...
                plt.scatter(nanog_peak_tmp, cell_cycle_tmp)
                plt.xlabel('Nanog peak')
                plt.ylabel('Cell cycle (frames)')
                cor = scipy.stats.pearsonr(nanog_peak_tmp, cell_cycle_tmp)
                plt.title(given_stack + ' R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
                ...
                plt.scatter(nanog_avg_tmp, cell_cycle_tmp)
                plt.xlabel('Nanog average')
                plt.ylabel('Cell cycle (frames)')
                cor = scipy.stats.pearsonr(nanog_avg_tmp, cell_cycle_tmp)
                plt.title(given_stack + ' R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
                ...
                # add stack nanog metrics to the global ones
                for item in nanog_on_tmp:
                    nanog_on.append(item)
                
                for item in nanog_peak_tmp:
                    nanog_peak.append(item)
                
                for item in nanog_avg_tmp:
                    nanog_avg.append(item)
                
                for item in cell_cycle_tmp:
                    cell_cycle.append(item)
                
                for item in nanog_traces_tmp:
                    nanog_traces.append(item)


""" plot global nanog analysis """

if nanog_analysis:

    if save_plots:
        pltn = 1 
        plot_directory = os.path.join(main_directory, 'Plots')
        if not os.path.exists(plot_directory):
            os.mkdir(plot_directory)    
        
    plt.scatter(nanog_on, nanog_peak)
    plt.xlabel('Nanog on frame')
    plt.ylabel('Nanog peak')
    cor = scipy.stats.pearsonr(nanog_on, nanog_peak)
    plt.title('R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
    ...
    plt.scatter(nanog_avg, nanog_peak)
    plt.xlabel('Nanog average')
    plt.ylabel('Nanog peak')
    cor = scipy.stats.pearsonr(nanog_avg, nanog_peak)
    plt.title('R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
    ...
    plt.scatter(nanog_on, nanog_avg)
    plt.xlabel('Nanog on frame')
    plt.ylabel('Nanog average')
    cor = scipy.stats.pearsonr(nanog_on, nanog_avg)
    plt.title('R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
    ...
    plt.scatter(nanog_on, cell_cycle)
    plt.xlabel('Nanog on frame')
    plt.ylabel('Cell cycle (frames)')
    cor = scipy.stats.pearsonr(nanog_on, cell_cycle)
    plt.title('R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
    ...
    plt.scatter(nanog_peak, cell_cycle)
    plt.xlabel('Nanog peak')
    plt.ylabel('Cell cycle (frames)')
    cor = scipy.stats.pearsonr(nanog_peak, cell_cycle)
    plt.title('R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
    ...
    plt.scatter(nanog_avg, cell_cycle)
    plt.xlabel('Nanog average')
    plt.ylabel('Cell cycle (frames)')
    cor = scipy.stats.pearsonr(nanog_avg, cell_cycle)
    plt.title('R = ' + str("%.4f" % cor[0]) +'; p = ' + str("%.4f" % cor[1]))
    ...
    # plot all 32-cell stage traces of nanog
    for trace in nanog_traces:
        plt.plot(trace[1], trace[0], linewidth = 0.5, color='Grey', linestyle='dashed')
    plt.xlabel('Hours since 32-cell division')
    plt.ylabel('Normalized Nanog')
    ...
    for trace in nanog_traces:
        plt.plot(trace[2], trace[0], linewidth = 0.5, color='Grey', linestyle='dashed')
    plt.xlabel('Hours until 64-cell division')
    plt.ylabel('Normalized Nanog')
    ...
    # write nanog global results
    with open(os.path.join(main_directory, 'nanog_analysis.csv'), 'w', newline='') as csvfile:
        header = ['nanog_on', 'nanog_peak', 'nanog_avg', 'cell_cycle']
        writer = csv.writer(csvfile)
        writer.writerow(header)
        for i in range(len(nanog_on)):
            writer.writerow([nanog_on[i], nanog_peak[i], nanog_avg[i], cell_cycle[i]])
