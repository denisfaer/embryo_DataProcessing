            ...
            if nanog_analysis:
                ...
                for pheno in pheno_data:
                    ...
                    for cell in temp_data:
                        ...
                        tau = 0
                        min_frame = cell[1][0][0]
                        while divs[0][0] - tau >= min_frame:
                            temp_nanog = []
                            temp_sox2 = []
                            temp_hist = []
                            if tau > max_tau_tmp:
                            ...
                            for frame in trace:
                                ...
                                if (frame[0] >= divs[0][0]) and (frame[0] <= divs[1][0]):
                                    temp_nanog.append(nanog)
                                
                                sox2 = (frame[2] - blue_min)/(blue_max - blue_min)
                                if (frame[0] >= (divs[0][0] - tau)) and (frame[0] <= (divs[1][0] - tau)):
                                    temp_sox2.append(sox2)
                                
                                hist = (frame[4] - black_min)/(black_max - black_min)
                                if (frame[0] >= (divs[0][0] - tau)) and (frame[0] <= (divs[1][0] - tau)):
                                    temp_hist.append(hist)
                                
                            cor = scipy.stats.pearsonr(temp_nanog, temp_sox2)
                            cell_correls.append([tau, cor[0]])
                            cell_determs.append([tau, cor[0] ** 2])
                            
                            cor = scipy.stats.pearsonr(temp_nanog, temp_hist)
                            cell_correlsh.append([tau, cor[0]])
                            cell_determsh.append([tau, cor[0] ** 2])
                            
                            tau += 4
                        
                        correls.append(cell_correls)
                        determs.append(cell_determs)
                        correlsh.append(cell_correlsh)
                        determsh.append(cell_determsh)
                
                # plot correlation as a function of delay
                correl_avg = []
                correl_stdev = []
                correl_min = []
                correl_max = []
                ...
                for t in range(int(max_tau_tmp / 4)):
                    correl_tau.append(t)
                    correl_tmp = []
                    for cell in correlsh:
                        if len(cell) >= (t + 1):
                            correl_tmp.append(cell[t][1])
                            
                    avg = scipy.stats.tmean(correl_tmp)
                    std = scipy.stats.tstd(correl_tmp)
                    correl_avg.append(avg)
                    correl_stdev.append(std)
                    correl_min.append(avg - std)
                    correl_max.append(avg + std)
                        
                plt.plot(correl_tau, correl_avg, color = 'Black')
                plt.fill_between(correl_tau, correl_max, correl_min, facecolor='Black', alpha=0.25)
                ...
                plt.plot(correl_tau, correl_avg, color = 'Green')
                plt.fill_between(correl_tau, correl_max, correl_min, facecolor='Green', alpha=0.25)
                
                plt.xlabel('Delay between curves (h)')
                plt.ylabel("Correlation coefficient (R)")
                plt.title('Correlation between Sox2/H2B and Nanog' + ' (' + given_stack + ')')
                plt.ylim(-1, 1)
                ...
                # plot determination as a function of delay
                correl_avg = []
                correl_stdev = []
                correl_min = []
                correl_max = []
                ...
                plt.title('Determination between Sox2/H2B and Nanog' + ' (' + given_stack + ')')
                plt.ylim(-0.1, 0.7)
                ...
                # save tau-correl files for global analysis
                if max_tau_tmp > max_tau:
                    max_tau = max_tau_tmp
                    
                for item in correls:
                    all_correls.append(item)
                
                for item in determs:
                    all_determs.append(item)
                
                for item in correlsh:
                    all_correlsh.append(item)
                
                for item in determsh:
                    all_determsh.append(item)
...
if nanog_analysis:
    ...
    for t in range(int(max_tau / 4)):
        correl_tau.append(t)
        correl_tmp = []
        for cell in all_correlsh:
            if len(cell) >= (t + 1):
                correl_tmp.append(cell[t][1])
    ...
    # write H2B vs Nanog global correlations
    with open(os.path.join(main_directory, 'H2B_nanog_correl.csv'), 'w', newline='') as csvfile:
        header = ['tau', 'avg', 'avg-stdev', 'avg+stdev']
        writer = csv.writer(csvfile)
        writer.writerow(header)
        for i in range(len(correl_tau)):
            writer.writerow([correl_tau[i], correl_avg[i], correl_min[i], correl_max[i]])
    ...
    # write Sox2 vs Nanog global correlations
    with open(os.path.join(main_directory, 'Sox2_nanog_correl.csv'), 'w', newline='') as csvfile:
        header = ['tau', 'avg', 'avg-stdev', 'avg+stdev']
        writer = csv.writer(csvfile)
        writer.writerow(header)
        for i in range(len(correl_tau)):
    ...
    plt.xlabel('Delay between curves (h)')
    plt.ylabel("Correlation coefficient (R)")
    plt.title('Correlation between Sox2/H2B and Nanog')
    plt.ylim(-1, 1)
    ...
    # plot determinations as a function of delay
    correl_avg = []
    correl_stdev = []
    correl_min = []
    correl_max = []
    ...
    # write H2B vs Nanog global determination
    with open(os.path.join(main_directory, 'H2B_nanog_determ.csv'), 'w', newline='') as csvfile:
        header = ['tau', 'avg', 'avg-stdev', 'avg+stdev']
        writer = csv.writer(csvfile)
        writer.writerow(header)
        for i in range(len(correl_tau)):
    ...
    # write Sox2 vs Nanog global determination
    with open(os.path.join(main_directory, 'Sox2_nanog_determ.csv'), 'w', newline='') as csvfile:
        header = ['tau', 'avg', 'avg-stdev', 'avg+stdev']
        writer = csv.writer(csvfile)
        writer.writerow(header)
        for i in range(len(correl_tau)):
    ...
    plt.xlabel('Delay between curves (h)')
    plt.ylabel("Coefficient of determination (R^2)")
    plt.title('Determination between Sox2/H2B and Nanog')
    plt.ylim(-0.1, 0.7)
