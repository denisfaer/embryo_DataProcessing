...
def transform(string): # transforms the lineage() string output to a dataframe
    ...
    while len(string) > 0:
        ...
        #print(sub)
        #print(sub[0:sub.find('_')])
        #print(sub[(1+sub.find('_')):len(sub)])
        out.append([int(sub[0:sub.find('_')]), int(sub[(1+sub.find('_')):len(sub)])])
        string = string.replace(sub + '<', '')
        
    #print('Internal out:', out)
    #print(np.flip(np.array(out), axis=0))
...
def find (frame, idi): # returns the short and long camera intensities for a given frame_ID; [-1;-1] if not found
    ...
    for i in range(len(short)):
        if frame == short[i][0] and idi == short[i][1]:
            ...
            #print('Searched for: ', frame, '-', idi)
            #print('Found: ',  short[i][0], '-',  short[i][1])
            #print('Found Intensities: ',  short[i][2], '-',  long[i][2])
            break
    ...
    for t in track:
        ...
        for i in range(len(histone)):
            if t[0] == histone[i][0] and t[1] == histone[i][1]:
                temph = histone[i][2]
                #print('Searched for: ', t[0], '-', t[1])
                #print('Found: ',  histone[i][0], '-',  histone[i][1])
                break
        if temp[0] > 0:
...
def make_plots(lineages, plotn): # make a set of plots for a set of tracked cells
    ...
    for i in range(cells):
        plt.scatter((stack[2][i][1][len(stack[2][i][1])-1, 2] - blue_min)/(blue_max - blue_min), (stack[2][i][1][len(stack[2][i][1])-1, 3] - red_min)/(red_max - red_min), s=dot_scale*(stack[2][i][1][len(stack[2][i][1])-1, 4] - black_min)/(black_max - black_min), c='Black', alpha=scatter_alpha)
...
for given_stack in given_stacks:
    ...
    out_dir = os.path.join(main_directory, given_stack)
    ...
    for cell in allcells:
        if trouble:
            ...
            print()
            #print(transform(lineage(cell[0], cell[1])))
...
""" pickle data """ ### THIS PART WILL BE DELETED
...
""" add plotting-only option """ ### THIS PART WILL BE REDONE
